...
func _run() -> void:
	...
	# The texture must be square for mipmaps to work correctly. The texture's in-memory size
	# is still pretty low (less than 200 KB), so this should not cause any performance problems.
	var data := PackedByteArray()
	for mipmap: int in [256, 128, 64, 32, 16, 8, 4, 2, 1]:
		for y: int in range(mipmap):
			for x: int in range(mipmap):
	...
	# Saving it binary significantly reduces the size of scenes using these addon nodes. It also
	# stops Godot complaining that the scenes are too large.
	ResourceSaver.save(texture, "res://addons/antialiased_line2d/antialiased_texture.res")
